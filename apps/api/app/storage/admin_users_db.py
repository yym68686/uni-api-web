...
def _to_item(
    row: User, *, keys_total: int, keys_active: int, sessions_active: int
) -> AdminUserItem:
...
def _with_counts_query(now: dt.datetime):
    keys_counts = (
        select(
...
async def list_admin_users(
    ...
    for user, keys_total, keys_active, sessions_active in rows:
        items.append(
            _to_item(
                user,
...
async def get_admin_user(
    ...
    now = dt.datetime.now(dt.timezone.utc)
    q = _with_counts_query(now).where(User.id == user_id)
    row = (await session.execute(q)).first()
    if not row:
        return None
    user, keys_total, keys_active, sessions_active = row
    return _to_item(
        user,
        keys_total=int(keys_total),
        keys_active=int(keys_active),
...
async def update_admin_user(
    ...
    item = await get_admin_user(session, user.id)
    if not item:
        return None
