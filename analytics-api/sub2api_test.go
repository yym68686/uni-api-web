...
func TestSubAccountLifecycleIsolationAndIdempotency(t *testing.T) {
	...
	if created.Load() != 3 || probes.Load() != int32(3*(len(subModels)+3)) || refreshed.Load() != 1 {
	...
	if created.Load() != 3 || probes.Load() != int32(5*(len(subModels)+3)) || refreshed.Load() != 1 {
	...
	service.subWorkOne(context.Background())
	if probes.Load() != int32(6*(len(subModels)+3)) {
	...
	if service.subWorkOne(context.Background()) || probes.Load() != int32(6*(len(subModels)+3)) {
	...
	service.subWorkOne(context.Background())
	store.db.QueryRow(`SELECT state FROM console_sub_accounts WHERE id=$1`, added.ID).Scan(&state)
	if state != "interrupted" || probes.Load() != int32(6*(len(subModels)+3)) {
