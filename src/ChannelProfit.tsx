...
export function ChannelProfit({
  ...
  const explanation = profit
    ? `仅取数值：${estimated} × ${salePercent}% × 6.9 − ${actual} = ¥${profit.amount}。利润率 = 利润 ÷（估算消费 × ${salePercent}% × 6.9）。${estimated === 0 || salePercent === 0 ? "收入为 0，利润率无法计算。" : ""}使用本行显示的消费口径，共用业务 Key 的渠道不能重复相加。`
    : "等待估算消费和渠道实际消费均可用后计算；未查询到的金额不按 0 处理。";
  return (
    <Tip text={explanation}>
      <div
        className={`channel-profit mono ${profit == null ? "muted" : profit.amount < 0 ? "negative" : profit.amount > 0 ? "profit-positive" : ""}`}
      >
