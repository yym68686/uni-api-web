...
import { chargesCacheWrite, displayedModelPrices, MODEL_PRICE_CATALOG } from "./modelPrices";
...
function PriceRow({ price, connection, onSaved }: { price: ModelPrice; connection: Connection; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      const next = { ...draft, source: "manual", charge_cache_write: chargesCacheWrite(draft) };
  ...
  return <article className="price-editor">
    ...
    <label className="price-verified price-cache-toggle"><input type="checkbox" aria-label={`${price.model} 计算缓存写入费用`} checked={chargesCacheWrite(draft)} disabled={pending} onChange={event => change({ charge_cache_write: event.target.checked })} />计算缓存写入费用</label>
...
export function PriceSettings({ prices, loading, error, connection, onSaved, refreshAction }: { prices: ModelPrice[]; loading: boolean; error?: string; connection: Connection; onSaved: () => void; refreshAction?: ReactNode }) {
  ...
  return <section className="data-panel prices-panel">
    ...
    <p className="field-note">GPT 默认不计缓存写入费用，其他模型默认计费；可逐项修改并保存。开启时，未单列的缓存写入按普通输入计价。不含搜索、缓存存储等额外费用，实际扣费以站点账单为准。</p>
