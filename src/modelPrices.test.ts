...
import { canonicalPriceModel, displayedModelPrices, MODEL_PRICE_CATALOG } from "./modelPrices";
...
it("shows exactly the detection models, preserves manual values and fills old placeholders", () => {
  ...
  expect(prices.map(p => p.model)).toEqual(SUB_MODELS);
  expect(prices.find(p => p.model === base.model)).toMatchObject({ input: 7, verified: false });
  expect(prices.filter(p => p.verified)).toHaveLength(20);
  const placeholder = { ...base, input: 0, output: 0, cache_read: 0, cache_write: 0, cache_write_1h: 0, verified: false, source: "fact-discovered" };
  expect(displayedModelPrices([placeholder]).find(p => p.model === base.model)).toMatchObject({ input: 2, output: 12, verified: true });
  expect(displayedModelPrices([]).find(p => p.model === "codex-auto-review")?.verified).toBe(false);
});
...
it("uses the longest complete model prefix, ignoring stale suffix prices and respecting unconfirmed bases", () => {
  ...
  expect(referencePrice(suffix.model, [suffix, { ...base, verified: false }])).toBeUndefined();
  expect(canonicalPriceModel("glm-5.3-flash-thinking")).toBe("glm-5.3-flash");
  expect(canonicalPriceModel("claude-fable-5-1-thinking")).toBe("claude-fable-5-1");
