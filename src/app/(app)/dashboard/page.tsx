...
export default async function DashboardPage() {
  const [locale, me, timeZone] = await Promise.all([getRequestLocale(), getCurrentUser(), getRequestTimeZone()]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={<DashboardKpisSkeleton />}>
        <DashboardKpis locale={locale} remainingCredits={remainingCredits} initialTimeZone={timeZone} />
      </Suspense>

      <div className="grid gap-4 lg:grid-cols-3">
        <Suspense fallback={<DashboardChartSkeleton />}>
          <DashboardChart locale={locale} initialTimeZone={timeZone} />
        </Suspense>
