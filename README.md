...
渠道观测的「利润」列按指定数值公式计算：`估算消费 × 2.5% × 6.9 − 渠道实际消费`，不对实际消费另做币种换算。第一行显示人民币 `¥金额`，第二行利润率为 `利润 ÷（估算消费 × 2.5% × 6.9）`。正利润绿色、负利润红色；缺少任一消费值时显示缺失，收入为零时利润率不可计算。导入渠道只使用当前窗口已完整同步的业务 Key 账单，不能以旧按日金额补算；其他渠道沿用本行实际消费可用的日历范围。
