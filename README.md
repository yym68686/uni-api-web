...
渠道检测已合并到「渠道观测」，无独立检测页面。搜索框同行点击「降智检测」只展开操作，按钮变为「一键检测」「取消检测」，每行显示「重新检测」。点击「取消检测」停止未完成的批量及单行检测并隐藏操作，已完成和上次检测结果保留；顺序调整草稿不受影响。检测使用观测页的来源、API key、模型、搜索、状态、余额、排序等筛选。筛选只决定渠道集合；检测固定请求 `gpt-6-astra` 的非流式 `/v1/responses`，提问「你的知识截止到哪年哪月？只答 YYYY-MM；不确定答未知。」每个来源的渠道去重后检测一次，一键检测覆盖全部匹配页，所有筛选渠道同时发起，可取消仍在进行的检测。
