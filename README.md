...
渠道管理的“添加到渠道”支持跨全部来源、全部已有 API key 接入的批量操作。编辑时，模型勾选、模型重命名、路由位置各有独立应用按钮，底部可应用全部设置；单项应用保留其余设置，位置单项使用调序接口，只移动目标 Key 已有的同名模型。新增模型或别名放到第 1 位；指定位置超出目标路由数量时落到末位，预览展示实际最终位置。已有接入页可“删除所有已保存接入”，保留基础渠道定义和其他渠道。

批量操作先完整读取来源和 Key 路由，再展示影响范围供确认；仅发现已保存接入，不创建其他 Key 的接入。站点分组按账号/分组及已确认绑定关联，原生渠道按现有名称和引擎分组合并来源。执行按来源版本逐项衔接，任一版本冲突、请求失败或结果不明即停止，显示已完成与未执行项，不自动重发；可停止后续操作，当前请求会等待结果。批量执行不是跨来源事务，已完成的操作保留；中途关闭网页会停止后续执行，重新打开后以各来源实时配置为准。
