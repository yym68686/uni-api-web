...
账户登录后从「渠道观测」切换到「渠道控制」，直接复用同一布局、筛选状态、指标、趋势和分页；默认全部来源立即显示表格，仅多一列临时控制。每行自动对应自己的来源。上下移动和停用先形成草稿，点击行内「应用」保存当前来源＋key＋模型范围的修改；「放弃」撤销草稿，表格上方的「撤销当前范围的全部临时修改」按来源分别清除已生效规则，并显示具体 API key、模型以及排序和停用数量。该操作覆盖整个标示范围，包含被其他表格筛选隐藏的渠道。
