...
固定使用 `gpt-6-astra`，通过 `/v1/responses` 发送流式 `say test`，从检测服务发起 HTTP 到首个 `response.output_text.delta` 记录首字延迟；要求成功完成事件及最终 assistant 文本，心跳、reasoning、HTTP 200 或截断连接不算成功。可用性成功后再次发送知识截止时间问题：仅最终文本唯一包含「未知」判通过、唯一包含「2024-06」判不通过，其余无法判定。可用性与此启发式规则分别显示，结果代表所选分组，不能确认其全部内部账号或模型身份。

后台 PostgreSQL 队列保存同步任务、检测进度、最后结果，网页刷新不影响任务。每实例最多两个账号并行、每账号两个分组并行；租约及任务 ID 阻止重复执行与过期结果覆盖。进程中断的付费请求不会自动重放，标记中断后可手动重试。支持账号、搜索、可用性、降智结果筛选，一键检测覆盖所有匹配页。新接口为 `/v1/sub2api/accounts`、`/v1/sub2api/accounts/{id}/sync`、`/v1/sub2api/accounts/{id}/stop`、`/v1/sub2api/checks`，仅账户会话可访问。
