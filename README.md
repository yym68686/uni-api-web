...
账号账单超过 5 页时，每轮扫描在第一页后额外读取一页最新账单（最多 1000 条，沿用站点实际分页上限，3 秒超时）。这些记录立即用于逐请求关联；该读取不会推进历史覆盖范围或分页游标，失败也不阻断原有完整扫描。完整扫描继续按时间正序恢复，重复记录按账号、上游 Key 和账单 ID 去重，不增加模型调用。`sub_account_spend_recent` 日志记录该步骤的行数、耗时及是否成功。
