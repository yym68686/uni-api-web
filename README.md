...
从 sub2api 检测导入的渠道，其「渠道实际消费」使用保存的 `routing_key_id` 业务 Key 查询站点账号 `/api/v1/usage` 日志，排除限额测试 Key。支持 5 分钟、15 分钟、1–24 小时、近 7/30 天、今天、本周、本月、今年与全部；日历范围使用控制台分析时区，滚动窗口与统计页面使用同一分钟起点及固定结束时间。按日志 `created_at` 精确筛选 `[from,to)`，使用十位小数金额累计 `actual_cost`，不从钱包余额变化或估算价格推算。

账单金额和时间存入独立 PostgreSQL 缓存，以账号、业务 Key、日志 ID 去重。首次查询或扩展窗口时按创建时间升序分页，游标持久保存；每批最多 5 页、20 秒，取消或重启后续读。窗口未全部覆盖时仅显示同步中/查询失败，不发布部分金额。完整缓存的重叠窗口直接复用；刷新重新读取最近一天以接收延迟账单或金额修正。多个 uni-api 来源/调用 Key/模型共用同一业务 Key 时只同步一次，金额是该业务 Key 整体消费；tooltip 明确提示不可重复相加，也不受模型、端点或流式筛选影响。未关联 sub2api 账号的普通渠道继续使用原有按日账单逻辑。


渠道观测的「利润」列按指定数值公式计算：`估算消费 × 对应模型售卖百分比 ÷ 100 × 6.9 − 渠道实际消费`，不对实际消费另做币种换算。第一行显示人民币 `¥金额`，第二行利润率为 `利润 ÷（估算消费 × 对应模型售卖百分比 ÷ 100 × 6.9）`。正利润绿色、负利润红色；缺少任一消费值时显示缺失，收入为零时利润率不可计算。导入渠道只使用当前窗口已完整同步的业务 Key 账单，不能以旧按日金额补算；其他渠道沿用本行实际消费可用的日历范围。
...
已完整匹配的原始渠道可读取账号余额、按选定时间范围读取原有 Key 的账单，并复用关联分组的可用模型及降智检测历史。多个 Key 按账号与 Key ID 去重，全部账单覆盖完整后才显示总额；分组检测历史也按账号和分组去重。原有 Key 历史从上游 Key 创建时间计算，不以控制台添加账号日期截断。相同 Key 在其他来源或调用 Key 中使用的消费仍属于同一业务 Key，不可重复相加。账号、站点或 key 匹配不完整/有歧义时不混用数据，抽屉显示关联状态。
