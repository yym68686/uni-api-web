...
账户登录后从「渠道观测」切换到「渠道控制」，直接复用同一布局、筛选状态、指标、趋势和分页；默认全部来源立即显示表格，仅多一列临时控制。每行自动对应自己的来源。上下移动和停用先形成草稿，点击行内「应用」保存当前来源＋key＋模型范围的修改；「放弃」撤销草稿，搜索框同行的「撤销当前范围的全部临时修改」清除已生效规则，悬停可查看具体来源、API key 和模型范围，不展示额外说明文字。有多个来源范围时，从按钮菜单中选择要撤销的一项；不会一次撤销其他来源。
