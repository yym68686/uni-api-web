...
后台 PostgreSQL 队列保存同步任务、检测进度、最后结果，网页刷新不影响任务。每实例最多两个账号并行、每账号两个分组并行；租约及任务 ID 阻止重复执行与过期结果覆盖。进程中断的付费请求不会自动重放，标记中断后可手动重试。支持账号、搜索、模型、可用性、降智结果、倍率上限筛选和倍率升降序；倍率选项来自其他筛选后的数据，选择后保留小于等于阈值的渠道。一键检测覆盖所有匹配页，只检测筛选出来的模型。新接口为 `/v1/sub2api/accounts`、`/v1/sub2api/accounts/{id}/sync`、`/v1/sub2api/accounts/{id}/stop`、`/v1/sub2api/checks`，仅账户会话可访问。
