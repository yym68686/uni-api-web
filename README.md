...
批量操作仅覆盖已保存接入，不创建其他 Key 的接入。站点分组按账号/分组及已确认绑定关联，原生渠道按现有名称和引擎分组合并来源。执行按来源版本逐项衔接，任一版本冲突、请求失败或结果不明即停止，显示已完成与未执行项，不自动重发；可停止后续操作，当前请求会等待结果。批量执行不是跨来源事务，已完成的操作保留；中途关闭网页会停止后续执行，重新打开后以各来源实时配置为准。
