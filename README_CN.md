...
- 账单：余额充值（Creem 银行卡 + ZhuPay 支付宝 / 微信支付）+ 余额变更历史
- Profile：账号信息 + 注销账号
...
支持的支付方式：
- Creem 银行卡支付（USD 结算）
- ZhuPay 支付宝 / 微信支付（CNY 结算）

后端需要配置环境变量：
- Creem：`CREEM_API_KEY`、`CREEM_PRODUCT_ID`、`CREEM_WEBHOOK_SECRET`
- ZhuPay：`ZHUPAY_PID`、`ZHUPAY_PRIVATE_KEY`、`ZHUPAY_PUBLIC_KEY`
- 公共项：`ZHUPAY_CNY_PER_CREDIT`（通过 ZhuPay 支付时，每 1 USD credit 实际收取多少人民币）、`APP_PUBLIC_URL`（例如 `http://localhost:3000`，用于银行卡支付回跳地址以及 ZhuPay 回调地址）

Webhook URL 需要指向前端（再薄转发到后端校验）：
- `POST <APP_PUBLIC_URL>/api/webhook/creem`
- `GET <APP_PUBLIC_URL>/api/webhook/zhupay`

本地开发如需接收任一支付平台回调，请使用可访问的公网隧道。
