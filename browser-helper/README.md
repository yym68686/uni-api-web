...
如果直接登录遇到 Turnstile，点击「使用浏览器登录」。助手直接打开原站、自动勾选登录协议并填写登录表单，再把成功登录的会话交回控制台。同步与检测由控制台继续执行。
...
安装时授予 HTTPS 站点访问权限，登录时不再打开授权中间页。
消息桥接仅在 uni-api 控制台运行；只有用户发起登录时才操作本次目标站点。
